...
@Composable
internal fun ProfileManager(
    controller: DolbyController,
    modifier: Modifier,
    onActivate: () -> Unit
) {
    ...
    val customProfiles = profiles.filter { it.custom }
    ...
    LazyColumn(
        ...
        if (customProfiles.isEmpty()) {
            ...
            items(customProfiles, key = { it.key }) { profile ->
                EqualizerPanel(Modifier.fillMaxWidth()) {
                    Column(
                        Modifier.padding(16.dp),
                        verticalArrangement = Arrangement.spacedBy(8.dp)
                        ...
                        Row(
                            ...
                            FilledTonalButton(
                                ...
                                Text(
                                    stringResource(
                                        if (active.key == profile.key) {
                                            R.string.dolby_profile_active
                                        } else {
                                            R.string.dolby_profile_activate
                                        }
                                    )
                                )
                            }
                            IconButton(onClick = { renaming = profile.key }) {
                                Icon(
                                    Icons.Default.Edit,
                                    stringResource(R.string.dolby_profile_rename)
                                )
                            }
    ...
    if (creating || renaming != null) {
        ...
        ProfileEditor(
            ...
            onSave = { name, base ->
                try {
                    if (existing == null) {
                        controller.createNamedProfile(name, base)
                    } else {
                        controller.renameNamedProfile(existing.key, name)
                    }
                    creating = false
                    renaming = null
                    null
    ...
    profiles.firstOrNull { it.key == deleting }?.let { profile ->
        AlertDialog(
            ...
            text = {
                Text(stringResource(R.string.dolby_profile_delete_message, profile.name))
            },
            confirmButton = {
                TextButton(
                    onClick = {
...
private fun ProfileEditor(
    ...
    var name by rememberSaveable(existing?.key) {
        mutableStateOf(existing?.name.orEmpty())
    }
    var base by rememberSaveable(existing?.key) {
        mutableIntStateOf(existing?.base ?: initialBase)
    }
    ...
    AlertDialog(
        ...
        title = {
            Text(
                stringResource(
                    if (existing == null) {
                        R.string.dolby_profile_create
                    } else {
                        R.string.dolby_profile_rename
                    }
                )
            )
        },
                ...
                if (existing == null) {
                    ...
                    ExposedDropdownMenuBox(
                        ...
                        OutlinedTextField(
                            ...
                            readOnly = true,
                            label = {
                                Text(stringResource(R.string.dolby_profile_base))
                            },
                            trailingIcon = {
